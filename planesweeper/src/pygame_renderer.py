...
class PygameRenderer(Renderer):
    ...
    def _render_asset(self, asset: Asset, pos: tuple[int, int]):
        if asset is not None and asset.path is not None:
            ...
            self._screen.blit(
                self._loaded_images[asset.key], pos)
    ...
    def _render_line(self, line: tuple[tuple[int, int], tuple[int, int], tuple[int, int, int]]):
        if line is not None:
